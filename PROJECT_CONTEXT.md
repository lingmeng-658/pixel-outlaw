...
- 新增 `src/contestedPickup.ts`，统一持有第二关争夺计划进度、延迟落地、争夺道具、主要抢夺者和相关计时状态。
- 落地警告、合法抢夺者筛选与替补、黄色标记、枪手有限拦截、拾取停留和生命周期清理由新模块集中管理。
- `MainScene` 保留最终敌人强化、完整枪手攻击、通用玩家拾取、敌人生成、第二关完成和区域流程；争夺数值、顺序、升级效果和反馈保持不变。
