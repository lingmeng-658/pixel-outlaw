...
- score、health、coins、area、levelCompleted、stage 和金币进度恢复。
- intro、heart、coffee、shield、final、clear 阶段起点恢复。
- Dusty Outskirts 通关出口与 Town Road 区域恢复。

### Save Loop Design Review

2026-07-22 已完成只读设计审查。结论：Continue 采用**阶段起点检查点恢复**，不恢复完整战场快照；保留分数、生命、金币、区域和第一关进度，但不恢复敌人、子弹、地面道具、玩家精确坐标或 Coffee / Shield 剩余时间。

最小实现应：

- 在 `src/save.ts` 增加 localStorage 读取、JSON 解析、字段与版本校验，并为损坏或不支持的存档提供安全回退。
- 在标题页仅对有效存档显示 Continue Saved Game。
- 保存并恢复 `area`、明确的关卡完成状态、`score`、`health`、`coins`、阶段及必要的金币掉落进度。
- 由阶段集中重建波次内部状态；区域出口作为关卡完成状态的派生结果，不单独持久化。
- 对当前 `version: 1` 存档提供明确兼容策略，避免直接类型断言或静默覆盖损坏数据。
