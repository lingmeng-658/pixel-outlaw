...
### 任务：实现最小 Continue Saved Game

目标：

- 扩展并校验版本化存档数据。
- 在标题页增加仅对有效存档可用的 Continue 入口。
- 按阶段起点恢复分数、生命、金币、区域和第一关进度。
- 安全处理缺失、损坏、旧版及不支持版本的存档。

不做什么：

- 不恢复完整战场快照。
- 不大范围重构 `MainScene` 或区域系统。
- 不新增商店、第二关或新道具。

验收标准：

- New Game 与 Continue 入口行为明确。
- 各阶段、已通关区域、金币和旧版/损坏存档均按设计恢复或安全回退。
- `npm run build` 通过，并完成存档与区域相关人工试玩。
