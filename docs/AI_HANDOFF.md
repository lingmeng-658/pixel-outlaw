...
### 2026-07-23 修复可复用关卡生命周期

根因：`clearLevelCompleteText()` 把视觉销毁与第二关完成、遭遇、争夺、临时强化和入口检查点重置混在一起；同时 Phaser restart 复用 Scene 实例，而原 `resetGameState()` 未完整重置整局字段，`townCheckpoint` 与可选 `autoStart` 参数又把 Restart 绑定到第二关并模糊了启动意图。

实际边界：

- `src/levelLifecycle.ts`：新增纯数据 `RunProgress`、`LevelEntryCheckpoint` 和可辨别 `SceneStartData`；`completedThrough` 使后续关卡检查点能保留此前完成进度，不绑定 Town Road。
- `src/main.ts`：`resetRunState()` 完整初始化新运行的基础资源、两关进度、检查点、遭遇、Town Road 流程、争夺控制器、临时武器/炸药、掉落与特殊敌人状态。
- `src/main.ts`：新增运行进度捕获/应用、通用入口检查点捕获/恢复、Restart 检查点选择和关卡进入适配；第一关 Restart 使用初始检查点，第二关首次进入未完成 Town Road 时捕获入口，完成或重进不覆盖。
- `src/main.ts`：场景启动明确区分 `title`、`newRun`、`restartLevel`；New Game、暂停/死亡 Restart、Save & Quit 和 Continue 走各自明确入口，不再使用 `townCheckpoint` 或可选布尔组合。
- `src/main.ts`：`clearLevelCompleteText()` 现在只停止 tween、销毁文字并清空引用，不再修改任何进度或运行时。
- `src/save.ts`：version 3 增加可选通用 `levelCheckpoint` 并做强类型校验；新存档保存精确入口快照，旧 v3 缺失时保守派生兼容检查点。未完成 Town Road 保存入口进度而非中途战场。
- `PROJECT_CONTEXT.md`：记录“运行进度 + 入口检查点 + 关卡运行时 + 纯视觉状态”模型与未来关卡接入方式。

验证结果：`npm run build` 通过，仅有既有 Vite 大 chunk 警告；`npm run dev` 成功启动（5173 已占用，Vite 自动使用 5174）后已停止；`git diff --check` 通过，完整 diff 与 status 已审查。尚未执行完整浏览器人工回归。

当前风险：旧 v3 存档从未保存过精确第二关入口资源，兼容路径只能保守回滚最大生命奖励并沿用可恢复资源；新存档可精确保存入口检查点。需人工覆盖完成后 Restart、两种 Save/Continue、回到 Dusty Outskirts 后 Restart，以及连续往返/重开无控制器残留。

范围确认：未实现第三关、注册表、多 Scene 或宽泛生命周期管理器；未修改敌人、批次、争夺、掉落、奖励数值或玩法。

提交状态：待使用 `fix: 修复可复用关卡生命周期` 提交并 push `main`。

### 2026-07-23 阶段 R2：拆分道具争夺控制

本阶段目标：纯重构第二关道具争夺边界，让计划、落地、主要抢夺者、有限拦截和拾取生命周期由独立模块统一控制。

实际修改：

- `src/contestedPickup.ts`：新增 `ContestedPickupController`，成为 `nextLevelTwoPickup`、`pendingContestedPickup`、`contestedPickup`、`primaryContender`、`contenderEnabledAt`、`contenderInterceptUntil` 的唯一状态来源。
- `src/contestedPickup.ts`：迁移计划进度判断、随机落点、落地警告与可取消延迟、合法抢夺者筛选、黄色标记跟随与替补、枪手有限拦截、接近/停留拾取和统一清理。
- `src/main.ts`：通过五个强类型且限于争夺边界的 hooks 提供敌人列表、玩家引用、争夺道具创建、最终敌人强化和完整枪手攻击入口；敌人强化实现、枪手攻击状态机、通用玩家拾取、敌人生成与第二关完成仍归 `MainScene`。
- `src/main.ts`：玩家抢先拾取、敌人销毁、清空道具、离区、Restart、正常完成和 `L` 调试完成均同步通知争夺控制器，延迟警告、黄色标记和对象引用可统一失效。
- `PROJECT_CONTEXT.md`：补充新模块职责和场景保留边界。

行为保持：配置数值、争夺计划顺序、落点算法、合法敌人类型、强化枪手排除规则、拦截截止时间从主要抢夺者分配时开始的语义、拾取停留、升级结果和反馈文字均未调整。

验证结果：`npm run build` 通过，仅有既有 Vite 大 chunk 警告；`npm run dev` 成功启动本地 Vite 后已停止；`git diff --check` 通过，完整 diff 与 status 已审查。尚未执行浏览器争夺流程人工试玩。

当前风险：需人工确认近战与枪手争夺观感、玩家抢取、主要抢夺者死亡替补、多次争夺，以及 Restart / 离区 / 正常完成 / `L` 调试完成时无残留。

提交状态：待使用 `refactor: 拆分道具争夺控制` 单独提交并与阶段 C 一起 push `main`。

### 2026-07-23 阶段 C：添加第二关完成调试快捷键

本阶段目标：为本地主机增加 `L` 快速完成 Town Road，并让调试完成与正常完成复用同一条具名完成路径。

实际修改：

- `src/main.ts`：键位注册新增 `L`，但仅由既有 localhost / 127.0.0.1 调试入口处理，不在正式操作提示中展示。
- `src/main.ts`：新增 `debugCompleteTownRoad()`，仅在游戏已开始、Town Road 未完成、非区域切换且非 Game Over 时清理敌人、双方子弹、道具、金币和争夺状态，再触发完成。
- `src/main.ts`：提取幂等的 `completeLevelTwo()`；正常击杀完成与 `L` 调试完成共同调用，统一停止遭遇、清理敌弹与道具、完成入口流程、发放一次生命奖励、重建生命 UI 并显示原完成标题。

验证结果：`npm run build` 通过，仅有既有 Vite 大 chunk 警告；`npm run dev` 成功启动本地 Vite 后已停止；`git diff --check` 通过，完整 diff 与 status 已审查。未执行浏览器按键与完整流程人工试玩。

范围确认：本阶段未开始争夺模块拆分，未修改第二关配置、完成条件、奖励数值、存档或线上正式操作提示。

提交状态：已单独提交为 `db8b07c feat: 添加第二关完成调试快捷键`，尚未 push。

### 2026-07-23 拆分第二关流程控制

本轮目标：纯重构 Town Road 准备区、战斗触发、返回入口和相关视觉清理职责，不改变玩法、坐标、数值、画面或存档语义。

实际修改：

- `src/townRoadFlow.ts`：新增 `TownRoadFlow`，作为战斗是否启动、南侧返回是否开放、战斗触发对象和返回提示对象的唯一状态来源；提供准备、尝试开战、完成、离区、返回判定和战斗状态查询等语义方法。
- `src/townRoadFlow.ts`：集中创建与销毁触发线、提示文字和返回入口提示；销毁 GameObject 前统一清理相关 tween，重复状态切换不会残留旧对象。
- `src/main.ts`：移除原四项流程状态及其视觉 helper；保留 `ContinuousEncounter` 启停与更新、实际区域切换、完成奖励、存档和 `AMBUSH STARTED` 浮动提示。
- `PROJECT_CONTEXT.md`：补充新模块职责和本轮重构边界。
- `src/levelTwo.ts`、`src/encounter.ts`、`src/save.ts`：已审阅，未修改。

验证结果：修改前与修改后 `npm run build` 均通过，仅有既有 Vite 大 chunk 警告；`npm run dev` 成功启动 Vite（本地地址 `http://localhost:5173/pixel-outlaw/`）后已正常停止；`git diff --check` 通过，完整 diff 与 status 已审查。

当前风险：尚未完成真实浏览器人工试玩；需重点确认准备阶段不出怪且可返回、越线只开战一次、开战后入口视觉与传送同步关闭、完成后入口恢复，以及 Restart / Continue 回到原有状态。

范围确认：未修改 `Y=420` 战斗触发线、`Y=542` 返回判定、出生点、入口宽度、敌人批次和战斗参数；未修改争夺、枪手弹道、强化、奖励或存档语义；未处理其他独立问题，也未继续拆分其他系统。

提交状态：待使用 `refactor: 拆分第二关流程控制` commit 并 push `main`。push 后停止，等待 ChatGPT 读取提交并审查。

### 2026-07-23 阶段 B：添加第二关安全准备区与战斗触发线

本轮目标：进入 Town Road 后先停留在不限时安全准备区，玩家主动向北越过触发线才启动第二关；开战时同步关闭南侧返回入口的视觉和传送判定。

实际修改：

- `src/levelTwo.ts`：新增 `combatEntry.triggerY / lineWidth` 与 `townRoadReturn.halfWidth / triggerY`，集中管理战斗触发线和南侧返回判定数值。
- `src/main.ts`：新增 `levelTwoCombatStarted`、`townRoadReturnOpen`，以及独立的触发线/返回入口视觉对象集合；`enterTownRoad()` 在未完成状态只重置运行时并显示准备提示，不再立即启动遭遇。
- `src/main.ts`：越线后由 `checkLevelTwoCombatTrigger()` 一次性清除提示、关闭南侧入口、启动 `ContinuousEncounter` 并显示 `AMBUSH STARTED`；`updateLevelTwo()` 在未开战时直接返回。
- `src/main.ts`：`canReturnFromTownRoad()` 集中控制返回判定，`setTownRoadReturnOpen()` 同步管理视觉；战斗完成后恢复既有南侧返回能力。Restart、未完成 Continue 和重新进入未完成 Town Road 都通过场景重建与 `resetLevelTwoRuntime()` 回到准备阶段。
- `docs/AI_HANDOFF.md`：同步实现、验证、风险和试玩清单。`src/encounter.ts` 与 `src/save.ts` 已审阅但未修改，存档结构和检查点语义保持不变。

初始化与清理：`resetGameState()` 初始化新增状态和对象引用；`resetLevelTwoRuntime()` 重置遭遇、战斗状态和触发提示；开战、返回 Dusty Outskirts、完成和场景重建均通过具名 helper 清理触发线及返回入口对象。

验证结果：`npm run build` 通过，仅有既有 Vite 大 chunk 警告；`git diff --check` 通过。未进行浏览器自动化或人工试玩。

重点试玩：进入 Town Road 后长时间停留无敌人/道具；准备阶段南侧正常返回；越线后首批开始且入口提示立即消失；退回南侧不能传送或重启遭遇；战斗中 Restart 和未完成 Continue 返回准备区并重开入口；通关后入口恢复，重新进入完成区域无触发线或重复战斗。

当前风险：触发线 `Y=500`、提示位置和南侧准备区空间需要真实画面确认；战斗完成后的返回入口按既有行为重新开放。未发现需要越界处理的独立问题。

范围确认：未实现商店、购买 UI、金币消费、边界感知走位、升级树或第三关；未修改敌人批次、生成速度、战斗参数、争夺规则、枪手弹道、存档版本或中途保存语义；未进行 `MainScene` 系统性拆分。

提交状态：待使用 `feat: 添加第二关战斗触发区` 提交并 push `main`。建议下一步由用户运行 `npm run dev` 完成上述真实试玩，并由 ChatGPT 重新读取实际提交后验收。
